...
The navigation milestone is intentionally read-only. It exposes Projects, a selected project's Features, and a minimal Feature destination without implementing creation, editing, context, generation, tabs, or menu workflows.
...
`PageStructure` owns the single `main` landmark. Its Header, Sidebar visibility, Sidebar width, accordion state, user menu, organization panel, and query observers survive child-route navigation. Route pages render content sections only.
...
- **Project group:** opening it enables the project's feature query. Its hover Go-to action navigates to the project route. Its horizontal three-dot menu is visible but intentionally inert.
- **Features node:** its Add action is always visible but intentionally inert; it performs no request. It has no List action because the project page owns the feature list.
- **Feature group:** its hover Go-to action navigates to the feature route. Its horizontal three-dot menu is intentionally inert. Expanding the group displays a future-navigation empty message.
