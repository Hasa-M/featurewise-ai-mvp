import { GitBranch, RefreshCw, Unplug } from 'lucide-react';
...
export function ProjectRepositoryPanel({
  ...
  const repositoryScope = query.data?.attemptExpiresAt ?? state ?? '';
  ...
  if (state === 'connecting')
    return (
      <EmptyState title='Connecting to GitHub'>
        Complete installation and authorization in GitHub. This attempt expires
        after ten minutes.
      </EmptyState>
    );
  if (state === 'disconnected')
    return (
      <EmptyState title='Connect a GitHub repository'>
        ...
        tokens.
        <Button
          leadingIcon={<GitBranch size={16} />}
          loading={mutations.attempt.isPending}
          onClick={() =>
            void mutations.attempt
              .mutateAsync()
    ...
    return (
      <Card className={styles.card} width='100%'>
        ...
        <Select
          ...
          placeholder={
            available.isPending ? 'Loading repositories...' : 'Select repository'
          }
        />
        ...
        <div className={styles.actions}>
          <Button
            ...
            onClick={() => mutations.connect.mutate(repositoryId)}
          >
            Connect repository
          </Button>
        </div>
